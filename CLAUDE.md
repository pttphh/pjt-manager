...
3. **PJT 세부화면 좌우 배치 = 좌 Tasks / 우 Todo (둘 다 `flex:1 1 0` — 폭 1:1)** (Task 사이드 패널이 우측에 열려도 좌측 Todo가 가려지지 않도록). **Todo 목록**: **todo.project_id = 이 PJT OR task.project_id = 이 PJT** 합집합(출처 PJT + 태그된 PJT 양쪽에 노출). 다른 PJT로 태그된 Todo엔 `→ 대상PJT명` 표기. 각 Todo에 🗑 삭제 버튼(확인창). **체크박스는 '체크(checked)'까지만 토글한다 — 여기서 완료(done)를 만들지 않는다.** 완료는 체크된 것을 모아 Todo 체크 탭에서 처리하는 것이 원칙. 전이: 미체크(draft·published)→`checked`(+`checked_at` 기록) / `checked`→해제→배포됐으면 `published`·미배포면 `draft`(+`checked_at` 지움) / `done`→해제(완료 취소)→**체크 이력이 있으면 `checked`** 복귀, 없으면 published·draft. **메모 유무로 판단하지 않는다**(메모 없이도 체크가 가능하므로 오판이 난다 — migrations/010).
