...
2. **Todo 체크 탭**: `todos.status in ('published','checked')`인 Todo만 표시 — draft 미노출, done 제거. 각 Todo 앞 **단일 상태 뱃지**: 배포(초록)=published → 체크(파랑)=checked. 상단 컨트롤은 좌→우로 **[보기 기준 토글] → [정렬 드롭박스] → [구분 드롭박스]**. **정렬**은 Task 작성일 기준 최신순(기본)/오래된순 — 그룹 순서와 그룹 내 Todo 순서에 모두 적용, 담당자별의 '미지정' 그룹만 정렬과 무관하게 항상 맨 아래. **구분 필터는 드롭박스**(Todo의 project_id → division 기준, '전체' + 구분 목록). **보기 기준** 토글 3종: [Task별](Task 단위 아코디언, 메타 `담당: …`) / [나의 할 일](담당자에 `lib/config.MY_NAME` 포함 Todo만, Task 그룹핑) / [담당자별](사람 단위 아코디언, 메타 `PJT: …`, 담당자 없으면 '미지정' 그룹). 구분 필터와 함께 동작. 미진행(published) 구간과 체크됨(checked) 구간 두 구간(한 그룹이 양 구간 동시 노출 가능). 미진행 Todo는 진행사항 메모 입력칸 **우측의 '저장' 버튼**으로 1건씩 누적 insert(상태 published 유지, 저장 후 칸이 비워져 다음 메모를 바로 이어 씀). 저장된 메모(날짜만)는 미진행 구간에 누적 표시되며, 각 메모 우측 **연필(✏️) 버튼으로 수정 모드 진입** → 인라인 입력 + **'저장'(내용 수정)·'삭제'(확인창 후 제거)·'✕'(취소)**. 완료되면 **별도 '체크' 버튼**으로 'checked' + 하단 이동(**메모 없이도 체크 가능**; 체크 시 입력 중이던 메모가 있으면 함께 저장). 체크됨 Todo는 최신 메모(날짜만) 표시 + **'체크 해제'(→ 'published' 복귀, `checked_at`도 null로 — 메모 이력은 유지)** / **'완료로 변경'(→ 'done', 화면 제거)** 두 버튼. Task의 전 Todo가 done이면 그룹도 제거.
