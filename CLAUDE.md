...
src/
  components/
    layout/   Sidebar.tsx('나의 할 일' 전용 — 규칙12. 드래그 리사이즈 220~400px, localStorage)
    ...
    task/     TaskModal.tsx          -- **좌측** 사이드 패널(drawer). ≥lg 폭 = 화면 왼쪽 끝 ~ 세부화면 Tasks 칸 오른쪽 끝(`widthPx` prop, zoom 보정한 CSS px) — 사이드바와 Tasks 칸은 덮되 **우측 Todo 는 가리지 않는다**, 최소 500px. 분할·좁은 화면(<lg)에선 전체화면(w-full). 바깥클릭 닫힘 없음(dirty면 확인창), X·취소만. decisions Ctrl+B 볼드
...
export interface Task {
  ...
  link_urls?: string[] | null                  // 관련 링크 여러 개 (migrations/009), 세부화면 Tasks 목록에서 ↗로 새 창 열림
...
export interface Todo {
  ...
  checked_at?: string | null   // 체크 단계를 거친 시각 (migrations/010). 체크 해제 시 null. done 해제 시 checked 복귀 판단 근거
...
2. **Todo 체크 탭**: **배포된 것만 보인다 — 담당자 무관.** `todos.status in ('published','checked')`만 표시하고 draft(미배포)·done은 제외. 각 Todo 앞 **단일 상태 뱃지**: 배포(초록)=published → 체크(파랑)=checked. 미진행 구간 = published, 체크됨 구간 = checked. 상단 컨트롤은 좌→우로 **[보기 기준 토글] → [정렬 드롭박스] → [구분 드롭박스]**. **정렬**은 Task 작성일 기준 최신순(기본)/오래된순 — 그룹 순서와 그룹 내 Todo 순서에 모두 적용, 담당자별의 '미지정' 그룹만 정렬과 무관하게 항상 맨 아래. **구분 필터는 드롭박스**(Todo의 project_id → division 기준, '전체' + 구분 목록). **보기 기준** 토글 3종: **[PJT별](기본값 = 이 탭의 메인 화면. Todo의 `project_id` 기준 PJT 단위 아코디언, 메타 `담당: …`)** / [Task별](Task 단위 아코디언, 메타 `담당: …`) / [담당자별](사람 단위 아코디언, 메타 `PJT: …`, 담당자 없으면 '미지정' 그룹) — '나의 할 일' 보기는 사이드바로 옮겨 삭제됨(규칙12). 구분 필터와 함께 동작. 한 그룹이 양 구간에 동시 노출 가능. 미진행 Todo는 진행사항 메모 입력칸 **우측의 '저장' 버튼**으로 1건씩 누적 insert(상태 published 유지, 저장 후 칸이 비워져 다음 메모를 바로 이어 씀). 각 Todo 제목 옆 **연필(✏️) 버튼으로 내용을 인라인 수정**(입력 + '저장'·'✕' 취소, Enter 저장·Esc 취소. 제목만 — 상태·담당자·PJT는 Task 창에서). 이 탭의 모든 변경(체크·완료·메모·내용 수정)은 `emitDataChanged()`로 사이드바에 통보해 '나의 할 일'이 즉시 갱신된다. 저장된 메모(날짜만)는 미진행 구간에 누적 표시되며, 각 메모 우측 **연필(✏️) 버튼으로 수정 모드 진입** → 인라인 입력 + **'저장'(내용 수정)·'삭제'(확인창 후 제거)·'✕'(취소)**. 완료되면 **별도 '체크' 버튼**으로 'checked' + 하단 이동(**메모 없이도 체크 가능**; 체크 시 입력 중이던 메모가 있으면 함께 저장). '체크됨' 섹션 헤더 우측에 **'✓ 모두 완료로 변경 (n)' 일괄 버튼**(확인창) — 대상은 **그때 화면에 보이는 체크됨 Todo**(구분 필터 적용 범위, 담당자별 보기의 중복은 제거), `checked_at`은 남겨 완료 취소 시 checked 로 복귀 가능. 체크됨 Todo는 최신 메모(날짜만) 표시 + **'체크 해제'(→ 'published' 복귀, `checked_at`도 null로 — 메모 이력은 유지)** / **'완료로 변경'(→ 'done', 화면 제거)** 두 버튼. Task의 전 Todo가 done이면 그룹도 제거.
3. **PJT 세부화면 좌우 배치 = 좌 Tasks / 우 Todo (둘 다 `flex:1 1 0` — 폭 1:1)** (Task 사이드 패널이 우측에 열려도 좌측 Todo가 가려지지 않도록). **Todo 목록**: **todo.project_id = 이 PJT OR task.project_id = 이 PJT** 합집합(출처 PJT + 태그된 PJT 양쪽에 노출). 다른 PJT로 태그된 Todo엔 `→ 대상PJT명` 표기. 각 Todo에 🗑 삭제 버튼(확인창). **체크된 Todo는 줄이 그어진 채 목록에 그대로 남고, 완료(done)된 Todo는 기본으로 접혀 있다**(`완료 n건` 토글로 펼침 — 쌓이면 화면을 덮으므로 감추되, 무엇을 끝냈는지 보고 잘못 완료한 것을 되돌릴 통로는 남긴다). **체크박스는 '체크(checked)'까지만 토글한다 — 여기서 완료(done)를 만들지 않는다.** 완료는 체크된 것을 모아 Todo 체크 탭에서 처리하는 것이 원칙. 전이: 미체크(draft·published)→`checked`(+`checked_at` 기록) / `checked`→해제→배포됐으면 `published`·미배포면 `draft`(+`checked_at` 지움) / `done`→해제(완료 취소)→**체크 이력이 있으면 `checked`** 복귀, 없으면 published·draft. **메모 유무로 판단하지 않는다**(메모 없이도 체크가 가능하므로 오판이 난다 — migrations/010).
4. **메모는 누적 저장**(todo_memos insert), 화면에는 최신 1건만 표시. 날짜만 표기(시각 없음).
5. **TaskModal은 단일 창**: 신규 등록·기존 Task 클릭 전부 동일 컴포넌트. 필드: Task명 / 날짜(기본 작성일) / 멤버(기본=PJT 멤버 전원) / 결정&전달사항 / **링크(여러 개, 새 창)** / Todo 행(내용·담당자·PJT·삭제). 링크는 PJT와 동일 패턴(link_urls 배열, migrations/009 미적용 시 폴백 저장), 세부화면 Tasks 목록에 각 링크 `↗`로 노출. 담당자 = Task 멤버 중 체크박스 복수 선택. Todo 상태는 이 창에 표기하지 않고 변경 불가. **새로 추가되는 Todo는 담당자와 무관하게 항상 draft로 생성**(기존 Todo 상태는 건드리지 않음). **모든 Todo는 배포 탭에서 배포를 거쳐야** 진행 단계로 넘어간다 — 담당자가 `MY_NAME`이라고 배포를 건너뛰지 않는다. 하단: 저장 + 삭제(확인창, 기타 Task는 삭제 불가)만 — **배포 버튼 없음**(배포는 배포 탭에서 Todo 단위/Task 일괄 처리).
6. **배포 탭**: 두 섹션 — 상단 **미배포**(draft Todo 있는 Task, 기본 펼침) / 하단 **배포됨**(draft 없이 published만, **기본 접힘**·헤더 클릭으로 펼침). 배포해도 회색으로 남아 되돌리기 가능, **모든 Todo가 checked/done으로 넘어가야 사라짐**(무한 누적 아님). 묶음 = 헤더 `Task명 (작성 M/D) — 프로젝트명`(draft 있으면 주황+'이 Task 전체 배포', 전부 배포됐으면 회색+'배포됨 n건·되돌리기 가능') + 지시사항(decisions) 미리보기 + **링크(Task의 `link_urls`, 있을 때만 노출 — 배포 시 지시사항과 함께 전달되는 참고 자료. `↗` 새 창)** + 전체 Todo 목록(draft=정상+'배포', published=회색+'미배포로 되돌리기', checked/done=회색 표시만). 각 Task 헤더 좌측 ▶/▼로 접기/펼치기(헤더 클릭 토글, '전체 배포' 버튼은 stopPropagation). 배포 시 `deployed_at=now()`, 되돌리기 시 null. PJT 세부 Tasks 목록 표기는 `YY.MM.DD Task명`.
7. **PJT 관리 탭**: 태그별 컬럼(그리드 auto-fill minmax(220px,1fr) 자동 줄바꿈 — 가로 스크롤 없음, 빈 컬럼도 표시), PJT명 카드. 복수 태그 = 중복 노출. 마지막에 '태그 없음' 컬럼. 카드 배경 = PJT 상태색(단, **긴급이거나 '내 미실행 Todo' 보유 시 빨강** — 색상 토큰 절 참조). 카드 이름 앞에 우선순위 아이콘(긴급🚨/중요💡/둘다🔥). 상단에 **상태 필터 칩(미진행·진행중·보류·완료 다중 선택, 기본값 = 완료 제외 3개)** — 칩 색이 곧 카드색 범례. 완료 PJT는 '완료' 칩을 켜면 함께 표시(설정에서도 열람 가능). 우측 상단 'PJT 등록'. 새 Task 작성 버튼은 이 탭/배포 탭에 없음(Task 작성은 PJT 세부화면에서만).
...
12. **사이드바 = '나의 할 일' 전용.** PJT 트리·정렬·필터 없음(PJT 진입은 PJT 관리 탭에서만 한다). 담당자에 `lib/config.MY_NAME`이 포함된 **배포된(`published`) Todo만** 표시한다 — 미배포(draft)는 배포 탭에서 배포해야 올라오고, checked·done은 제외(체크 이후는 Todo 체크 탭에서 관리). 배포된 것만 오므로 상태 뱃지는 두지 않는다. 순서는 **활동일 오래된순 고정** — 활동일 = **마지막 진행사항 메모(`todo_memos.created_at`), 없으면 Todo 등록일(`todos.created_at`)**. 진행사항을 적으면 그 날짜로 갱신되어 아래로 내려가므로, 오래 방치된 일만 위에 남는다. 카드에 표시하는 날짜도 이 활동일이다(정렬과 표시가 어긋나지 않게). 각 항목 = Todo명 + `PJT명 · M/D`. **클릭 시 점프**: `navigate('/main', { state: { focusTodoId } })` → MainPage가 Todo 체크 탭을 열고 `focusTodoId`를 넘김 → TodoCheckTab이 그 Todo가 속한 그룹을 펼치고 스크롤·강조(2.6초). 스크롤은 `behavior:'smooth'` 금지(렌더가 멈춘 탭에서 아예 동작하지 않음) — 즉시 스크롤 + 재시도.
...
```
- **PJT 상태색(단일 소스 = `types.STATUS_CARD_STYLE`)**: 미진행=회색(#F1F0EC/#55534E) · 진행중=파랑(#E6F1FB/#0C447C) · 보류=노랑(#FAEEDA/#633806) · 완료=초록(#E1F5EE/#085041). **긴급(is_urgent) 체크 시 상태 무관 빨강(#FCEBEB/#791F1F)** — `types.projectColor(status, urgent, myOpenTodo?)` 사용. PJT 카드·세부 상태 드롭박스 등 상태색 쓰는 모든 곳 일괄.
- **내 미실행 Todo 빨강(PJT 관리 카드 전용)**: 담당자에 `lib/config.MY_NAME`이 포함된 **done 아닌** Todo(draft·published·checked)를 가진 PJT는 긴급과 같은 빨강. 판정 기준은 **`todo.project_id`**(Todo가 속한 PJT, task.project_id 아님). `projectColor`의 3번째 인자로 넘긴다 — 세부화면 드롭박스는 이 규칙을 쓰지 않는다(2번째 인자까지만).
