...
src/
  components/
    ...
    task/     TaskModal.tsx          -- 우측 사이드 패널(drawer). ≥lg 폭 = 세부화면 Tasks 칸 실측폭+28px(`widthPx` prop)이라 그 칸을 정확히 덮는다, 최소 500px. 분할·좁은 화면(<lg)에선 전체화면(w-full). 바깥클릭 닫힘 없음(dirty면 확인창), X·취소만. decisions Ctrl+B 볼드
...
3. **PJT 세부화면 좌우 배치 = 좌 Todo / 우 Tasks (둘 다 `flex:1 1 0` — 폭 1:1)** (Task 사이드 패널이 우측에 열려도 좌측 Todo가 가려지지 않도록). **Todo 목록**: **todo.project_id = 이 PJT OR task.project_id = 이 PJT** 합집합(출처 PJT + 태그된 PJT 양쪽에 노출). 다른 PJT로 태그된 Todo엔 `→ 대상PJT명` 표기. 각 Todo에 🗑 삭제 버튼(확인창). 체크박스 자유 체크/해제: 체크→'done', 해제→**체크 단계를 거쳤으면(`checked_at` 있음) 'checked'**, 아니면 배포됐으면 'published'·미배포면 'draft'. **메모 유무로 판단하지 않는다**(메모 없이도 체크가 가능하므로 오판이 난다 — migrations/010).
