...
src/
  components/
    ...
    task/     TaskModal.tsx          -- 우측 사이드 패널(drawer, ≥lg 500px — 세부화면 Tasks 칸과 같은 폭). 분할·좁은 화면(<lg)에선 전체화면(w-full). 바깥클릭 닫힘 없음(dirty면 확인창), X·취소만. decisions Ctrl+B 볼드
...
2. **Todo 체크 탭**: `published`·`checked` 전부 + **`draft`는 담당자에 `MY_NAME`이 포함된 것만** 표시(자기 자신에게 배포할 이유가 없으므로 — 규칙12와 짝). done 제거. 각 Todo 앞 **단일 상태 뱃지**: 미배포(회색)=draft → 배포(초록)=published → 체크(파랑)=checked. 미진행 구간 = draft+published, 체크됨 구간 = checked. 상단 컨트롤은 좌→우로 **[보기 기준 토글] → [정렬 드롭박스] → [구분 드롭박스]**. **정렬**은 Task 작성일 기준 최신순(기본)/오래된순 — 그룹 순서와 그룹 내 Todo 순서에 모두 적용, 담당자별의 '미지정' 그룹만 정렬과 무관하게 항상 맨 아래. **구분 필터는 드롭박스**(Todo의 project_id → division 기준, '전체' + 구분 목록). **보기 기준** 토글 2종: [Task별](Task 단위 아코디언, 메타 `담당: …`) / [담당자별](사람 단위 아코디언, 메타 `PJT: …`, 담당자 없으면 '미지정' 그룹) — '나의 할 일' 보기는 사이드바로 옮겨 삭제됨(규칙12). 구분 필터와 함께 동작. 한 그룹이 양 구간에 동시 노출 가능. 미진행 Todo는 진행사항 메모 입력칸 **우측의 '저장' 버튼**으로 1건씩 누적 insert(상태 published 유지, 저장 후 칸이 비워져 다음 메모를 바로 이어 씀). 저장된 메모(날짜만)는 미진행 구간에 누적 표시되며, 각 메모 우측 **연필(✏️) 버튼으로 수정 모드 진입** → 인라인 입력 + **'저장'(내용 수정)·'삭제'(확인창 후 제거)·'✕'(취소)**. 완료되면 **별도 '체크' 버튼**으로 'checked' + 하단 이동(**메모 없이도 체크 가능**; 체크 시 입력 중이던 메모가 있으면 함께 저장). 체크됨 Todo는 최신 메모(날짜만) 표시 + **'체크 해제'(→ 'published' 복귀, `checked_at`도 null로 — 메모 이력은 유지)** / **'완료로 변경'(→ 'done', 화면 제거)** 두 버튼. Task의 전 Todo가 done이면 그룹도 제거.
3. **PJT 세부화면 좌우 배치 = 좌 Todo / 우 Tasks** (Task 사이드 패널이 우측에 열려도 좌측 Todo가 가려지지 않도록). **Todo 목록**: **todo.project_id = 이 PJT OR task.project_id = 이 PJT** 합집합(출처 PJT + 태그된 PJT 양쪽에 노출). 다른 PJT로 태그된 Todo엔 `→ 대상PJT명` 표기. 각 Todo에 🗑 삭제 버튼(확인창). 체크박스 자유 체크/해제: 체크→'done', 해제→**체크 단계를 거쳤으면(`checked_at` 있음) 'checked'**, 아니면 배포됐으면 'published'·미배포면 'draft'. **메모 유무로 판단하지 않는다**(메모 없이도 체크가 가능하므로 오판이 난다 — migrations/010).
4. **메모는 누적 저장**(todo_memos insert), 화면에는 최신 1건만 표시. 날짜만 표기(시각 없음).
5. **TaskModal은 단일 창**: 신규 등록·기존 Task 클릭 전부 동일 컴포넌트. 필드: Task명 / 날짜(기본 작성일) / 멤버(기본=PJT 멤버 전원) / 결정&전달사항 / **링크(여러 개, 새 창)** / Todo 행(내용·담당자·PJT·삭제). 링크는 PJT와 동일 패턴(link_urls 배열, migrations/009 미적용 시 폴백 저장), 세부화면 Tasks 목록에 각 링크 `↗`로 노출. 담당자 = Task 멤버 중 체크박스 복수 선택. Todo 상태는 이 창에 표기하지 않고 변경 불가. **새 Todo는 draft로 생성 — 단 담당자에 `MY_NAME`이 있으면 처음부터 `published`+`deployed_at`**(`lib/todoStatus.initialTodoStatus`, 규칙12). 저장 시 **draft인 기존 Todo에 `MY_NAME`이 담당자로 들어오면 published로 승격**한다(published·checked·done 상태는 건드리지 않음). 하단: 저장 + 삭제(확인창, 기타 Task는 삭제 불가)만 — **배포 버튼 없음**(배포는 배포 탭에서 Todo 단위/Task 일괄 처리).
...
12. **사이드바 = '나의 할 일' 전용.** PJT 트리·정렬·필터 없음(PJT 진입은 PJT 관리 탭에서만 한다). 담당자에 `lib/config.MY_NAME`이 포함된 **`draft`·`published` Todo만** 표시하고 checked·done은 제외(체크 이후는 Todo 체크 탭에서 관리). 순서는 **Task 작성일 오래된순 고정**. 각 항목 = 미배포/배포 뱃지 + Todo명 + `PJT명 · M/D`. **클릭 시 점프**: `navigate('/main', { state: { focusTodoId } })` → MainPage가 Todo 체크 탭을 열고 `focusTodoId`를 넘김 → TodoCheckTab이 그 Todo가 속한 그룹을 펼치고 스크롤·강조(2.6초). 스크롤은 `behavior:'smooth'` 금지(렌더가 멈춘 탭에서 아예 동작하지 않음) — 즉시 스크롤 + 재시도.
