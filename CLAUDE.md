...
3. **PJT 세부화면 좌우 배치 = 좌 Tasks / 우 Todo (둘 다 `flex:1 1 0` — 폭 1:1)** (Task 사이드 패널이 우측에 열려도 좌측 Todo가 가려지지 않도록). **Todo 목록**: **todo.project_id = 이 PJT OR task.project_id = 이 PJT** 합집합(출처 PJT + 태그된 PJT 양쪽에 노출). 다른 PJT로 태그된 Todo엔 `→ 대상PJT명` 표기. 각 Todo에 🗑 삭제 버튼(확인창). 체크박스 자유 체크/해제: 체크→'done', 해제→**체크 단계를 거쳤으면(`checked_at` 있음) 'checked'**, 아니면 배포됐으면 'published'·미배포면 'draft'. **메모 유무로 판단하지 않는다**(메모 없이도 체크가 가능하므로 오판이 난다 — migrations/010).
